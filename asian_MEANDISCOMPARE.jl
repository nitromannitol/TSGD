...
theta_grid = linspace(-3, 3,int(1e3));
mean1Array = Float64[];
mean2Array = Float64[]; 
...
for theta in theta_grid
	...
	push!(mean1Array, mean(guessArray));
	push!(mean2Array, mean(guessArray2));
end

plot(theta_grid, mean1Array, label = "gradient average");
plot(theta_grid, mean2Array, label = "displacement average");
legend();
xlabel("theta");
savefig("XXXX.png");
